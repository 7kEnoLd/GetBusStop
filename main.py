...
def get_location(city, linename, mykey):
    print("请耐心等待")
    url_api = 'https://restapi.amap.com/v3/bus/linename?city={}&keywords={}&key={}&extensions=all'.format(city, linename, mykey)
    ...
    return [[stop[i]['id'], stop[i]['name'], linename, stop[i]['sequence'], stop[i]['location']] for i in range(len(stop))]


def process_buslines(keyword_parts, linename, rt):
    def match_criteria(name, criteria):
        base, temp = criteria
        if '路' not in base[0]:  # 选中第一个分割部分
            base += '路'
        if temp:
            return base[0] + '(' in name and '临时' in name
        else:
            return base[0] + '(' in name
    ...
    for index, line in enumerate(buslines):
        name = line['name']
        if any(match_criteria(name, [base, True]) for base in split_keywords):
            return index
    ...
    if not found:
        for index, line in enumerate(buslines):
            name = line['name']
            if any(match_criteria(name, [base, False]) for base, _ in split_keywords):
                return index
